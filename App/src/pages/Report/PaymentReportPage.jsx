...
function RecentSection({ data, isMobile }) {
  ...
  return <Section icon={<HistoryRoundedIcon />} title="Recent Payments">{isMobile ? <Stack spacing={1}>{data.recent.map((payment) => <Paper key={payment.id} variant="outlined" sx={{ p: 1.25, borderRadius: 1.5 }}><Box sx={{ display: "flex", justifyContent: "space-between", gap: 1 }}><Typography sx={{ fontSize: 15, fontWeight: 700 }}>{payment.type}</Typography><Typography sx={{ color: payment.amount < 0 ? "#e11d25" : "#00a94f", fontSize: 15, fontWeight: 800 }}>{money(payment.amount)}</Typography></Box><Typography sx={{ mt: .7, fontSize: 13, fontWeight: 650 }}>{payment.source || "—"}</Typography><Typography color="text.secondary" sx={{ mt: .3, fontSize: 12.5 }}>{new Date(payment.paidAt).toLocaleString()}</Typography><Box sx={{ mt: .5, display: "flex", gap: 1.5 }}><Typography sx={{ fontSize: 13 }}>{payment.method}</Typography><Typography color="text.secondary" sx={{ fontSize: 13 }}>{payment.actor?.name || "Unassigned/System"}</Typography></Box></Paper>)}</Stack> : <Box sx={{ overflow: "hidden", border: "1px solid #d8e2ef", borderRadius: 1.5 }}><TableHeader columns={["Type", "Source", "Date & Time", "Method", "Staff", "Amount"]} />{data.recent.map((payment, index) => <Box key={payment.id} sx={{ minHeight: 66, px: 2, display: "grid", gridTemplateColumns: "repeat(6,minmax(0,1fr))", alignItems: "center", borderTop: index ? "1px solid #d8e2ef" : 0 }}><Stack direction="row" spacing={1.4} sx={{ alignItems: "center" }}><Box sx={{ width: 38, height: 38, display: "grid", placeItems: "center", borderRadius: "50%", color: payment.amount < 0 ? "#ef151d" : "#00a94f", bgcolor: payment.amount < 0 ? "#ffe2e5" : "#dcf8e9" }}>{payment.amount < 0 ? <ArrowDownwardRoundedIcon /> : <ArrowUpwardRoundedIcon />}</Box><Typography sx={{ fontSize: 15, fontWeight: 650 }}>{payment.type}</Typography></Stack><Typography sx={{ fontSize: 15 }}>{payment.source || "—"}</Typography><Typography sx={{ fontSize: 14.5 }}>{new Date(payment.paidAt).toLocaleString()}</Typography><Typography sx={{ fontSize: 15 }}>{payment.method}</Typography><Typography sx={{ fontSize: 15 }}>{payment.actor?.name || "Unassigned/System"}</Typography><Typography sx={{ color: payment.amount < 0 ? "#e11d25" : "#00a94f", fontSize: 15, fontWeight: 750 }}>{money(payment.amount)}</Typography></Box>)}</Box>}</Section>;
}
...
function Value({ label, value }) { return <Box sx={{ minWidth: 0 }}><Typography color="text.secondary" sx={{ fontSize: 11.5, lineHeight: 1.25 }}>{label}</Typography><Typography sx={{ mt: .3, fontSize: 13, lineHeight: 1.3, fontWeight: 650, overflowWrap: "anywhere" }}>{value}</Typography></Box>; }
