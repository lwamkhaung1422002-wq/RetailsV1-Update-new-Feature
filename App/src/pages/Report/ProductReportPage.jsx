...
export default function ProductReportPage() {
  ...
  const reportQuery = useMemo(() => {
    const today = yangonDateKey();
    const range = dateRange === "all" ? { from: productReportStartDate, to: today } : dateRange === "today" ? { from: today, to: today } : { from, to };
    return { ...range, ...(tab !== "Overview" && search ? { search } : {}), ...(category !== "All categories" ? { categoryId: category } : {}) };
  }, [category, dateRange, from, search, tab, to]);
  ...
  const reportData = useMemo(() => reportResult || { summary: { totalProducts: 0, totalQuantity: 0, lowStockCount: 0, outOfStockCount: 0 }, products: [], topSellers: [], slowSellers: [] }, [reportResult]);
  ...
  return <Box sx={{ maxWidth: 1500, mx: "auto", py: 1 }}>
    ...
    <Box sx={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 2, mb: 2.5 }}>
      <SummaryCard label="Total Products" value={String(reportData.summary.totalProducts)} helper="Active Products" />
      <SummaryCard label="Total Quantity" value={String(reportData.summary.totalQuantity)} helper="In Stock" />
      <SummaryCard label="Low Stock Items" value={String(reportData.summary.lowStockCount)} helper="Need Attention" />
      <SummaryCard label="Out of Stock Items" value={String(reportData.summary.outOfStockCount)} helper="Currently Out" />
    </Box>

    {detailKey ? <DetailedReportTable kind={detailKey} rows={detailRows} /> : <Box sx={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(560px, 1fr))", gap: 2.25 }}>{visibleReports.map(({ key, ...report }) => <ReportTable key={key} kind={key} {...report} onViewAll={() => setTab({ top: "Top Seller", slow: "Slow Seller", low: "Low Stock", out: "Out of Stock" }[key])} />)}</Box>}
    ...
    <Popover open={Boolean(dateAnchor)} anchorEl={dateAnchor} onClose={() => setDateAnchor(null)} anchorOrigin={{ vertical: "bottom", horizontal: "right" }} transformOrigin={{ vertical: "top", horizontal: "right" }} slotProps={{ paper: { sx: { width: 370, p: 2, borderRadius: 2 } } }}>
      <Typography sx={{ fontSize: 17, fontWeight: 600, mb: 1.5 }}>Date and time</Typography>
...
function MobileProductFilter({ anchor, onOpen, onClose, dateRange, setDateRange, from, setFrom, to, setTo, category, setCategory }) {
  ...
  return <><IconButton aria-label="Filter product reports" onClick={onOpen} sx={{ color: "inherit" }}><FilterAltRoundedIcon /></IconButton><Popover open={Boolean(anchor)} anchorEl={anchor} onClose={onClose} anchorOrigin={{ vertical: "bottom", horizontal: "right" }} transformOrigin={{ vertical: "top", horizontal: "right" }} slotProps={{ paper: { sx: { width: "calc(100vw - 28px)", maxWidth: 420, p: 2, borderRadius: 2 } } }}><Typography sx={{ fontSize: 18, lineHeight: 1.3, fontWeight: 600 }}>Filters</Typography><Typography color="text.secondary" sx={{ mt: 1.75, mb: .8, fontSize: 12.5, lineHeight: 1.25, fontWeight: 600 }}>DATE AND TIME</Typography><Box sx={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: .75 }}>{["all", "today", "custom"].map((range) => <Button key={range} variant="outlined" onClick={() => setDateRange(range)} sx={{ minWidth: 0, minHeight: 40, px: .5, borderColor: dateRange === range ? "primary.main" : "divider", bgcolor: dateRange === range ? "#eaf3ff" : "transparent", color: dateRange === range ? "primary.main" : "text.primary", textTransform: "uppercase", fontSize: 11.5, fontWeight: dateRange === range ? 600 : 500 }}>{range}</Button>)}</Box>{dateRange === "custom" && <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1, mt: 1.25 }}><TextField type="date" label="From" size="small" value={from} onChange={(event) => setFrom(event.target.value)} slotProps={{ inputLabel: { shrink: true } }} /><TextField type="date" label="To" size="small" value={to} onChange={(event) => setTo(event.target.value)} slotProps={{ inputLabel: { shrink: true } }} /></Box>}<Typography color="text.secondary" sx={{ mt: 2, mb: .8, fontSize: 12.5, lineHeight: 1.25, fontWeight: 600 }}>CATEGORY</Typography><TextField select fullWidth size="small" value={category} onChange={(event) => setCategory(event.target.value)} sx={{ "& .MuiOutlinedInput-root": { minHeight: 42, fontSize: 14 } }}>{categories.map((item) => <MenuItem key={item.id} value={item.id}>{item.name}</MenuItem>)}</TextField><Stack direction="row" justifyContent="flex-end" spacing={1} sx={{ mt: 2 }}><Button onClick={() => { setDateRange("all"); setFrom(""); setTo(""); setCategory("All categories"); }} sx={{ minHeight: 40, textTransform: "uppercase", fontSize: 12.5, fontWeight: 500 }}>Reset</Button><Button variant="contained" onClick={onClose} sx={{ minHeight: 40, textTransform: "uppercase", fontSize: 12.5, fontWeight: 600 }}>Apply</Button></Stack></Popover></>;
}
...
function MobileProductTable({ title, columns, rows, onViewAll }) { return <Card sx={{ border: "1px solid", borderColor: "divider", borderRadius: 2, boxShadow: "0 2px 8px rgba(15,23,42,.05)" }}><CardContent sx={{ p: 1.5, "&:last-child": { pb: 1.5 } }}><Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 1, mb: 1 }}><Typography sx={{ fontSize: 15.5, lineHeight: 1.3, fontWeight: 500 }}>{title}</Typography><Button size="small" onClick={onViewAll} sx={{ minWidth: 0, px: .5, textTransform: "none", fontSize: 11, fontWeight: 500 }}>View all</Button></Box><Stack spacing={.65}>{rows.map((row, index) => <Box key={row[0]} sx={{ display: "grid", gridTemplateColumns: "24px minmax(0, 1fr) auto", alignItems: "center", gap: .6, py: .85, borderTop: index ? "1px solid" : 0, borderColor: "divider" }}><Typography color="text.secondary" sx={{ fontSize: 11, lineHeight: 1.25, fontWeight: 400 }}>{index + 1}</Typography><Box><Typography noWrap sx={{ fontSize: 12.5, lineHeight: 1.3, fontWeight: 500 }}>{row[0]}</Typography><Typography color="text.secondary" sx={{ mt: .25, fontSize: 11.5, lineHeight: 1.3, fontWeight: 400 }}>{columns[2]}: {row[1]}</Typography></Box><Typography noWrap sx={{ fontSize: 11.5, lineHeight: 1.3, fontWeight: 500, textAlign: "right" }}>{row[2]}</Typography></Box>)}</Stack></CardContent></Card>; }
...
function MobileInventoryStatus() {
  ...
  return <Card sx={{ mt: 1.25, border: "1px solid", borderColor: "divider", borderRadius: 2, boxShadow: "0 2px 8px rgba(15,23,42,.05)" }}><CardContent sx={{ p: 1.5, "&:last-child": { pb: 1.5 } }}><Typography sx={{ fontSize: 17, fontWeight: 600 }}>Inventory Summary by Status</Typography><Box sx={{ display: "grid", gridTemplateColumns: "118px minmax(0, 1fr)", alignItems: "center", gap: 1.25, mt: 1.25 }}><Box sx={{ position: "relative", width: 110, height: 110, borderRadius: "50%", background: "conic-gradient(#50b982 0 90.2%, #f6b55c 90.2% 97.7%, #ef6767 97.7% 100%)", "&::after": { content: '""', position: "absolute", inset: 18, borderRadius: "50%", bgcolor: "background.paper" } }}><Box sx={{ position: "absolute", inset: 0, zIndex: 1, display: "grid", placeItems: "center", textAlign: "center" }}><Box><Typography sx={{ fontSize: 20, fontWeight: 700 }}>342</Typography><Typography color="text.secondary" sx={{ fontSize: 10.5, fontWeight: 400 }}>Total Quantity</Typography></Box></Box></Box><Stack spacing={.8}>{rows.map(([label, , quantity, percentage, color]) => <Box key={label}><Box sx={{ display: "flex", alignItems: "center", gap: .6 }}><Box sx={{ width: 9, height: 9, borderRadius: "50%", bgcolor: color }} /><Typography sx={{ fontSize: 13.5, fontWeight: 600 }}>{label}</Typography></Box><Typography color="text.secondary" sx={{ ml: 1.9, fontSize: 12, fontWeight: 400 }}>{quantity} pcs · {percentage}</Typography></Box>)}</Stack></Box><Box sx={{ mt: 1.4, border: "1px solid", borderColor: "divider", borderRadius: 1, overflow: "hidden" }}>{rows.map(([label, items, quantity, percentage], index) => <Box key={label} sx={{ display: "grid", gridTemplateColumns: "minmax(88px, 1.15fr) repeat(3, minmax(42px, .7fr))", gap: .2, px: .7, minHeight: 36, alignItems: "center", borderTop: index ? "1px solid" : 0, borderColor: "divider" }}><Typography sx={{ fontSize: 12.5, fontWeight: 600 }}>{label}</Typography><Typography sx={{ fontSize: 12.5, textAlign: "right", fontWeight: 500 }}>{items}</Typography><Typography sx={{ fontSize: 12.5, textAlign: "right", fontWeight: 500 }}>{quantity}</Typography><Typography sx={{ fontSize: 12.5, textAlign: "right", fontWeight: 500 }}>{percentage}</Typography></Box>)}<Box sx={{ display: "grid", gridTemplateColumns: "minmax(88px, 1.15fr) repeat(3, minmax(42px, .7fr))", gap: .2, px: .7, minHeight: 36, alignItems: "center", borderTop: "1px solid", borderColor: "divider", bgcolor: "#f7f9fc" }}><Typography sx={{ fontSize: 12.5, fontWeight: 600 }}>Total</Typography><Typography sx={{ fontSize: 12.5, textAlign: "right", fontWeight: 600 }}>18</Typography><Typography sx={{ fontSize: 12.5, textAlign: "right", fontWeight: 600 }}>342</Typography><Typography sx={{ fontSize: 12.5, textAlign: "right", fontWeight: 600 }}>100%</Typography></Box></Box></CardContent></Card>;
}
...
function ReportTable({ title, columns, rows, kind, onViewAll }) {
...
function DetailedReportTable({ kind, rows }) {
  ...
  return <Card sx={{ minWidth: 0, border: "1px solid", borderColor: "divider", borderRadius: 2, boxShadow: "0 2px 9px rgba(15,23,42,.05)" }}>
    <CardContent sx={{ p: 2.25, "&:last-child": { pb: 2.25 } }}>
      <Box sx={{ mb: 1.75 }}><Typography sx={{ fontSize: 19, fontWeight: 600 }}>{config.title}</Typography></Box>
...
function InventoryStatusCard() {
  return <Card sx={{ mt: 2.25, border: "1px solid", borderColor: "divider", borderRadius: 2, boxShadow: "0 2px 9px rgba(15,23,42,.05)" }}><CardContent sx={{ p: 2.5, "&:last-child": { pb: 2.5 } }}><Typography sx={{ fontSize: 18, fontWeight: 600, mb: 2 }}>Inventory Summary by Status</Typography><Box sx={{ display: "grid", gridTemplateColumns: "290px minmax(0, 1fr)", alignItems: "center", gap: 3 }}><Box sx={{ display: "flex", alignItems: "center", gap: 2 }}><Box sx={{ position: "relative", width: 142, height: 142, flexShrink: 0, borderRadius: "50%", background: "conic-gradient(#50b982 0 90.2%, #f6b55c 90.2% 97.7%, #ef6767 97.7% 100%)", "&::after": { content: '""', position: "absolute", inset: 22, bgcolor: "background.paper", borderRadius: "50%" } }}><Box sx={{ position: "absolute", inset: 0, zIndex: 1, display: "grid", placeItems: "center", textAlign: "center" }}><Box><Typography sx={{ fontSize: 20, fontWeight: 700 }}>342</Typography><Typography color="text.secondary" sx={{ fontSize: 11.5, fontWeight: 400 }}>Total Quantity</Typography></Box></Box></Box><Stack spacing={0.75}>{[["#50b982", "In Stock", "309 (90.2%)"], ["#f6b55c", "Low Stock", "26 (7.6%)"], ["#ef6767", "Out of Stock", "7 (2.1%)"]].map(([color, label, value]) => <Box key={label} sx={{ display: "grid", gridTemplateColumns: "10px auto", columnGap: .75, alignItems: "center" }}><Box sx={{ width: 8, height: 8, borderRadius: "50%", bgcolor: color }} /><Typography sx={{ fontSize: 12.5, fontWeight: 600 }}>{label}<Typography component="span" color="text.secondary" sx={{ ml: .5, fontSize: 12.5, fontWeight: 400 }}>{value}</Typography></Typography></Box>)}</Stack></Box><Box sx={{ border: "1px solid", borderColor: "divider", borderRadius: 1.5, overflow: "hidden" }}><StatusRow status="In Stock" items="14" quantity="309" percentage="90.2%" /><StatusRow status="Low Stock" items="3" quantity="26" percentage="7.6%" /><StatusRow status="Out of Stock" items="1" quantity="7" percentage="2.1%" /><StatusRow status="Total" items="18" quantity="342" percentage="100%" last /></Box></Box></CardContent></Card>;
}
