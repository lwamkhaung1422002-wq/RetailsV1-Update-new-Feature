...
export default function ReportPage() {
  ...
  if (!isMobile) return <Box sx={{ maxWidth: 1440, mx: "auto", py: 1 }}><Typography sx={{ fontSize: 30, fontWeight: 700 }}>{t("Reports")}</Typography><Typography color="text.secondary" sx={{ mt: .75, fontSize: 15 }}>{t("Review sales performance and product inventory insights.")}</Typography><Box sx={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 2.5, mt: 3 }}><ReportChoice icon={<AssessmentRoundedIcon />} title={t("Sale Report")} description={t("Review sales, payments, order activity, and store performance.")} label={t("Open report")} onClick={() => navigate("/report/sales")} /><ReportChoice icon={<Inventory2RoundedIcon />} title={t("Product Report")} description={t("Monitor product sales, slow-moving inventory, low stock, and out-of-stock items.")} label={t("Open report")} onClick={() => navigate("/report/products")} /></Box></Box>;
  if (isMobile) return <Box sx={{ px: 2, py: 2.5 }}><Typography sx={{ fontSize: 22, fontWeight: 800 }}>{t("Reports")}</Typography><Typography color="text.secondary" sx={{ mt: .5, fontSize: 14 }}>{t("Review your store performance and inventory insights.")}</Typography><Box sx={{ display: "grid", gap: 1.5, mt: 2.5 }}><MobileReportChoice icon={<AssessmentRoundedIcon />} title={t("Sale Report")} description={t("Sales, payments, orders, cost, and profit insights.")} onClick={() => navigate("/report/sales")} /><MobileReportChoice icon={<Inventory2RoundedIcon />} title={t("Product Report")} description={t("Product sales, low stock, and inventory movement insights.")} onClick={() => navigate("/report/products")} /></Box></Box>;
