...
export default function ReportPage() {
  ...
  if (!isMobile) return <Box sx={{ maxWidth: 1440, mx: "auto", py: 1 }}><Typography sx={{ fontSize: 30, fontWeight: 700 }}>Reports</Typography><Typography color="text.secondary" sx={{ mt: .75, fontSize: 15 }}>Review sales performance and product inventory insights.</Typography><Box sx={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 2.5, mt: 3 }}><ReportChoice icon={<AssessmentRoundedIcon />} title="Sale Report" description="Review sales, payments, order activity, and store performance." onClick={() => navigate("/report/sales")} /><ReportChoice icon={<Inventory2RoundedIcon />} title="Product Report" description="Monitor product sales, slow-moving inventory, low stock, and out-of-stock items." onClick={() => navigate("/report/products")} /></Box></Box>;
  if (isMobile) return <Box sx={{ px: 2, py: 2.5 }}><Typography sx={{ fontSize: 22, fontWeight: 800 }}>Reports</Typography><Typography color="text.secondary" sx={{ mt: .5, fontSize: 14 }}>Review your store performance and inventory insights.</Typography><Box sx={{ display: "grid", gap: 1.5, mt: 2.5 }}><MobileReportChoice icon={<AssessmentRoundedIcon />} title="Sale Report" description="Sales, payments, orders, cost, and profit insights." onClick={() => navigate("/report/sales")} /><MobileReportChoice icon={<Inventory2RoundedIcon />} title="Product Report" description="Product sales, low stock, and inventory movement insights." onClick={() => navigate("/report/products")} /></Box></Box>;
