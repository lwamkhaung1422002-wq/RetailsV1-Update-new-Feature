...
export default function StaffAccessPage() {
  ...
  const openAdd = () => { setForm(emptyForm); setSaveError(""); setDialog({ mode: "add" }); };
  const openEdit = (member) => { setForm({ name: member.user.name, email: member.user.email, password: "", role: member.role, active: member.active }); setSaveError(""); setDialog({ mode: "edit", member }); };
  const closeDialog = () => { if (!saving) setDialog(null); };
  const saveStaff = async () => {
    setSaving(true);
    setSaveError("");
    try {
      if (dialog.mode === "add") await api.staff.add(form);
      else await api.staff.update(dialog.member.id, { role: form.role, active: form.active });
  ...
  const togglePermission = async (key, enabled) => {
    ...
    setSaveError("");
    try {
      await api.staff.updatePolicy(selectedRole, next);
    ...
    <>
      ...
      {tab === 0 ? (
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "220px minmax(0,1fr)", gap: 2 }}>
          <FormControl fullWidth><InputLabel>Role</InputLabel><Select label="Role" value={selectedRole} onChange={(event) => setSelectedRole(event.target.value)}>{roles.map((role) => <MenuItem key={role} value={role}>{roleLabels[role]}</MenuItem>)}</Select></FormControl>
