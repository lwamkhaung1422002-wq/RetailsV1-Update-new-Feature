      ...
      <CardContent sx={{ p: 2.5, "&:last-child": { pb: 2.5 } }}>
        <Box sx={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) auto", columnGap: 1.75, alignItems: "start" }}>
          <Box sx={{ minWidth: 0 }}>
            ...
            <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 1, mt: 3 }}><Typography color="text.secondary" sx={{ fontSize: 18, lineHeight: 1.35 }}>{record.reason}{record.supplierName ? ` • Supplier: ${record.supplierName}` : ""}{record.invoiceReference ? ` • Invoice / Reference: ${record.invoiceReference}` : ""}{record.staffName ? ` • Staff: ${record.staffName}` : ""}</Typography>{returnInvoice && <Chip label="RETURN" variant="outlined" sx={{ height: 42, flexShrink: 0, borderRadius: 999, borderColor: statusColor, color: statusColor, bgcolor: "background.paper", fontWeight: 700, "& .MuiChip-label": { px: 1.5, fontSize: 16 } }} />}</Box>
          </Box>
