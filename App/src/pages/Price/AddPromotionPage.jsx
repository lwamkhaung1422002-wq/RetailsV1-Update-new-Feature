...
import { AppBar, Box, Button, Chip, FormControl, FormControlLabel, IconButton, InputLabel, MenuItem, Paper, Radio, RadioGroup, Select, TextField, Toolbar, Typography } from "@mui/material";
import ArrowBackRoundedIcon from "@mui/icons-material/ArrowBackRounded";
import CheckRoundedIcon from "@mui/icons-material/CheckRounded";
import { Alert } from "@mui/material";
import { usePosApi } from "../../hooks/useApiResource";
import { useCategoriesQuery, usePromotionCampaignsQuery } from "../../hooks/usePosQueries";
...
export default function AddPromotionPage() {
  ...
  const [form, setForm] = useState({ name: "", start: "", end: "", percentage: "", manualPrice: "", reason: "", audienceType: "ALL" });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      setForm({ name: campaign.name || promotion.name || "", start: promotion.startsAt ? new Date(promotion.startsAt).toISOString().slice(0, 10) : "", end: promotion.endsAt ? new Date(promotion.endsAt).toISOString().slice(0, 10) : "", percentage: promotion.type === "PERCENTAGE" ? String(promotion.value) : "", manualPrice: promotion.type === "FIXED_PRICE" ? String(promotion.value) : "", reason: promotion.reason || "Promotion update", audienceType: promotion.audienceType || "ALL" });
    }, 0);
    return () => window.clearTimeout(timer);
  }, [campaign, editing]);
  const selectedCategoryName = categories.find((item) => item.id === category)?.name;
  const calculatedPrice = selectedProduct && form.percentage !== "" ? Math.round(selectedProduct.price * (1 - Number(form.percentage) / 100)) : "";
  const shownPrice = form.manualPrice || (calculatedPrice ? String(calculatedPrice) : "");
  const shownPercentage = selectedProduct && form.manualPrice && Number(selectedProduct.price) > 0 ? ((selectedProduct.price - Number(form.manualPrice)) / selectedProduct.price * 100).toFixed(1) : form.percentage;
  ...
  const selectProduct = (product) => { setSelectedProduct(product); if (!editing) setForm((current) => ({ ...current, percentage: "", manualPrice: "" })); setError(""); };
  const changeProduct = () => { setSelectedProduct(null); setForm((current) => ({ ...current, percentage: "", manualPrice: "" })); setError(""); };
  const changeScope = (value) => { setScope(value); if (value !== "individual") changeProduct(); };
  const initialProductId = editing && campaign?.scope === "PRODUCT" ? campaign.promotions?.[0]?.productId : null;
  const showForm = scope !== "individual" || Boolean(selectedProduct);
  const loadError = categoriesError || campaignsError || (editing && campaignData && !campaign ? new Error("Promotion was not found.") : null);
  const save = async () => { const name = form.name.trim(); const reason = form.reason.trim(); if (saving) return; if (!editing && scope === "individual" && !selectedProduct) { setError("Select a product."); return; } if (name.length < 2 || !form.start || !form.end || reason.length < 3) { setError("Name, dates, and a reason of at least 3 characters are required."); return; } if (!editing && scope === "category" && !category) { setError("Select a category."); return; } const type = form.manualPrice ? "FIXED_PRICE" : "PERCENTAGE"; const value = Number(form.manualPrice || form.percentage); if (!Number.isFinite(value) || value <= 0 || (type === "PERCENTAGE" && value > 100)) { setError("Enter a valid discount."); return; } const startsAt = yangonDateTime(form.start); const endsAt = yangonDateTime(form.end, true); if (Number.isNaN(startsAt.valueOf()) || endsAt <= startsAt) { setError("End date must be after start date."); return; } setSaving(true); setError(""); try { const payload = { name, type, value, startsAt: startsAt.toISOString(), endsAt: endsAt.toISOString(), reason, timeZone: "Asia/Yangon", channel: "ALL", audienceType: form.audienceType, minimumQuantity: 1, discountBase: "REGULAR_PRICE" }; if (editing) await api.pricing.updatePromotionCampaign(campaign.id, { ...payload, expectedVersion: campaign.version }); else await api.pricing.createPromotionCampaign({ ...payload, scope: scope === "individual" ? "PRODUCT" : scope.toUpperCase(), ...(scope === "individual" ? { productId: selectedProduct.id } : {}), ...(scope === "category" ? { categoryId: category } : {}), state: "SCHEDULED" }); await Promise.all([queryClient.invalidateQueries({ queryKey: queryKeys.promotionCampaigns(shop?.id) }), queryClient.invalidateQueries({ queryKey: queryKeys.pricing(shop?.id) }), queryClient.invalidateQueries({ queryKey: queryKeys.products(shop?.id) })]); void Promise.all([queryClient.invalidateQueries({ queryKey: queryKeys.dashboard(shop?.id) }), queryClient.invalidateQueries({ queryKey: ["shops", shop?.id, "reports"] })]); navigate("/price?tab=promotion"); } catch (err) { setError(apiErrorMessage(err)); } finally { setSaving(false); } };
  ...
  return <Box sx={{ minHeight: "100dvh", pb: 12, bgcolor: "#f8fafc", fontFamily: "Inter, Roboto, 'Noto Sans Myanmar', sans-serif" }}>
    ...
    <Box sx={{ p: 2.5, maxWidth: 620, mx: "auto" }}>
      ...
      <Paper elevation={0} sx={scopePaperSx}><RadioGroup row value={scope} onChange={(event) => changeScope(event.target.value)} sx={scopeGroupSx}><FormControlLabel value="individual" disabled={editing} control={<Radio />} label="Individual" sx={scopeOptionSx} /><FormControlLabel value="category" disabled={editing} control={<Radio />} label="Category" sx={scopeOptionSx} /><FormControlLabel value="all" disabled={editing} control={<Radio />} label="All" sx={scopeOptionSx} /></RadioGroup></Paper>
      ...
      {scope === "individual" && <ProductSearchSelector selectedProduct={selectedProduct} onSelect={selectProduct} onChange={changeProduct} initialProductId={initialProductId} allowChange={!editing} />}
      {showForm && <><TextField fullWidth label="Promotion name" value={form.name} onChange={update("name")} placeholder="e.g. August discount" sx={{ mt: 2.5 }} />
      <TextField select fullWidth label="Audience" value={form.audienceType} onChange={update("audienceType")} sx={{ mt: 2 }}>
        <MenuItem value="ALL">All Customers</MenuItem><MenuItem value="RETAIL">Retail Only</MenuItem><MenuItem value="WHOLESALE">Wholesale Only</MenuItem>
      </TextField>
    ...
    <Paper elevation={5} sx={footerSx}><Box sx={{ display: "grid", gridTemplateColumns: "0.85fr 1.4fr", gap: 1.5 }}><Button variant="outlined" onClick={() => navigate("/price")} sx={cancelSx}>Cancel</Button><Button variant="contained" disabled={saving || (scope === "individual" && !selectedProduct)} startIcon={<CheckRoundedIcon />} onClick={save} sx={saveSx}>{saving ? "Saving…" : editing ? "Save Promotion" : "Create Promotion"}</Button></Box></Paper>
  </Box>;
}
