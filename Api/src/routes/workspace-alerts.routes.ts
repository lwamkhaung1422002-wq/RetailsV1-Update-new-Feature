...
workspaceAlertsRouter.get("/:shopId/payment-history", async (request, response, next) => { try {
  ...
  if (request.query.view === "history") {
    ...
    const records = [
      ...salePayments.map((payment) => ({ id: payment.id, apiId: payment.orderId, paymentId: payment.id, kind: payment.amount < 0 ? "sale-payment-refund" : "sale-payment", name: "Sale", invoice: payment.order?.orderNumber || payment.orderId, status: payment.amount < 0 ? "Refund" : "Paid", amount: Math.abs(payment.amount), method: payment.method, occurredAt: payment.paidAt, reason: payment.amount < 0 ? payment.reason || payment.note || "Sale payment refunded" : undefined })),
